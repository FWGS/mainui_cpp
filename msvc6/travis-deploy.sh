...
.. )| cut -d ' ' -f 1-3,5-100 >> README.md
 echo \`\`\` >> README.md
 echo >> README.md
 echo [Code on GitHub]\(https://github.com/$TRAVIS_REPO_SLUG/tree/$TRAVIS_COMMIT\) >> README.md
 echo >> README.md
 echo [Full changelog for this build]\(https://github.com/$TRAVIS_REPO_SLUG/commits/$TRAVIS_COMMIT\) >> README.md
 echo >> README.md
 for arg in $*; do
